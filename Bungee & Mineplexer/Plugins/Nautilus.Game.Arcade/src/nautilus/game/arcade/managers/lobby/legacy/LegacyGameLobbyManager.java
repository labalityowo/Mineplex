...
public class LegacyGameLobbyManager extends LobbyManager
{
	...
	public LegacyGameLobbyManager(ArcadeManager manager)
	{
		super(
				manager,
				// Missions
				new Location(WORLD, -2, 61, 3, -145, 0),
				null,
				// Spawn
				new Location(WORLD, 0, 61, 0),
				// Amp Stand
				new Location(WORLD, 0, 60, 11)
		);

		setGameText(new Location(WORLD, 0, 90, 50));
		setKitText(new Location(WORLD, -40, 80, 0));
		setTeamText(new Location(WORLD, 40, 80, 0));
		setAdvText(new Location(WORLD, 0, 100, -60));

		setPodium(new Location(WORLD, 0, 60, -11), Material.EMERALD_BLOCK.getId(), (byte) 0);

		_kitDisplay = new Location(WORLD, -17, 60, 0);

		float yaw = UtilAlg.GetYaw(UtilAlg.getTrajectory(_kitDisplay, getSpawn()));
		yaw = Math.round(yaw / 90) * 90;
		_kitDisplay.setYaw(yaw);

		_teamDisplay = new Location(WORLD, 18, 60, 0);
